...
# Make sure that the movies are sorted by selected way
Then /movies should be sorted by (.*)/ do |sort_by|
	Movie.all(:order => sort_by).each_cons(2) do |movie, next_movie|
		step %{I should see "#{movie.title}" before "#{next_movie.title}"}
	end
end
